...
void kernel_fnnls_mv(
 ...
 /*   
    if (tx==0) {
        auto sum{static_cast<T>(0)};
        auto const atb = Atb(real_ty);
        #pragma unroll
        for (int i=0; i<VECTOR_SIZE; ++i)
            sum += AtA(real_ty, i) * x(i);
        shrW[ty] = atb - sum;
    }
    __syncthreads();
    */
...
//        printf("1) w_max = %f w_max_idx = %d\n", w_max, w_max_idx);
    ...
    if (tx==0 && ty==0) {
        ...
        for (unsigned int i=VECTOR_SIZE-blockDim.y; i<VECTOR_SIZE; i++) {
