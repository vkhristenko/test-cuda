...
void kernel_compute_gradient() {
    ...
    if (row == 0) {
        ...
        if (w_max < eps) {
            // we are finished for this channel
            fnnls_finished[ch] = true;
            return;
        }

        // if not finished for this channel,
...
template<typename T, int SIZE>
__global__
void kernel_cholesky_solver_2d_root() {
    // indices
    int const tx = threadIdx.x;
    int const ty = threadIdx.y;
    int const gch = threadIdx.z + blockDim.z*blockIdx.x;
    int const lch = threadIdx.z;

    // shared mem
    extern __shared__ char smem[];
    T* __shrL = reinterpret_cast<T*>(smem);
    T* __shrLSums = __shrL + blockDim.z * SIZE * SIZE; 
    my_matrix_t<T> shrL{__shrL + lch * SIZE*SIZE};
    my_matrix_t<T> shrLSums{__shrLSums + lch*SIZE*SIZE};

    // map around eigen matrices
    my_matrix_t<T> AtA{AtAs[gch].data()};

    // retrieve the number of passive values
    auto npassive = g_npassive[ch]
}*/

template<typename T, int SIZE>
__global__
void kernel_cholesky_solver_old() {
    ...
    if (npassive == SIZE) 
        ...
        // this guy needs to continue
        // update the counter atomically and set its id
...
void kernel_fnnls_launcher() {
    ...
    for (int iter=0; iter<max_iterations; ++iter) {
        ...
        // this is the number of channels that still need to be used
        // for another iteration of fnnls
        channelsLeft = *pChannelsLeft;
