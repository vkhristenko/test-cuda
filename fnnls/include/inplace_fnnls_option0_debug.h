...
template<typename T>
__device__
void inplace_fnnls_debug(matrix_t<T> const& A,
                       vector_t<T> const& b,
                       vector_t<T>& x) {
...
#pragma unroll
          ...
          */
          ...
          sw.tail(nactive) = sAtb.tail(nactive) - (sAtA * sx).tail(nactive);
...
//          __syncthreads();
          ...
          const auto max_w = sw.tail(nactive).maxCoeff(&w_max_idx);
          ...
          sAtA.col(npassive).swap(sAtA.col(w_max_idx));
          sAtA.row(npassive).swap(sAtA.row(w_max_idx));
...
//          __syncthreads();
          while (npassive > 0) {
              ss.head(npassive) = sAtA
                  ...
                  .llt()
                  .solve(sAtb.head(npassive));

              if (ss.head(npassive).minCoeff() > 0.) {
                  sx.head(npassive) = ss.head(npassive);
                  break;
              }
              ...
              for (auto i = 0; i<npassive; ++i) {
                  if (ss(i) <= 0.) {
                      auto const ratio = sx(i) / (sx(i) - ss(i));
                      if (ratio < alpha) {
                          alpha = ratio;
                          alpha_idx = i;
                      }
              ...
              sx.head(npassive) += alpha*(ss.head(npassive) - sx.head(npassive));
              sx(alpha_idx) = 0;
              --npassive;

              sAtA.col(npassive).swap(sAtA.col(alpha_idx));
              sAtA.row(npassive).swap(sAtA.row(alpha_idx));
              ...
              Eigen::numext::swap(permutation[npassive], permutation[alpha_idx]);
